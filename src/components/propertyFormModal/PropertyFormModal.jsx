...
const PropertyFormModal = ({ show, onHide, property }) => {
        ...
        try {
            ...
            switch (propertyType) {
                ...
                case 'emergencyService':
                    response = await emergencyService.createService(data);
                    break;
                default:
    ...
    const renderSpecificFields = () => {
        switch (propertyType) {
            case 'accommodation':
                return <AccommodationFields formData={formData} handleChange={handleChange} />;
            case 'restaurant':
                return <RestaurantFields formData={formData} handleChange={handleChange} />;
            case 'bodyOfWater':
                return <BodyOfWaterFields formData={formData} handleChange={handleChange} />;
            case 'emergencyService':
                return <EmergencyServiceFields formData={formData} handleChange={handleChange} />;
            default:
                return null;
        }
    };
    ...
    return (
        <Modal show={show} onHide={onHide} centered backdrop="static" size="xl" contentClassName="property-form-modal-content">
            <Modal.Header closeButton className="modal-header-custom">
                <Modal.Title>{isEditing ? "Editar Propiedad" : "Agregar Nueva Propiedad"}</Modal.Title>
            </Modal.Header>
            <Form onSubmit={handleSubmit}>
                <Modal.Body>
                    {!isEditing && (
                        <Form.Group controlId="propertyTypeSelector" className="mb-4">
                            <Form.Label>Selecciona el tipo de propiedad que deseas crear</Form.Label>
                            <Form.Select value={propertyType} onChange={handlePropertyTypeChange} className="property-type-selector">
                    ...
                    {isEditing && (
                        <p>Aquí irá el formulario para editar la propiedad.</p>
                    )}
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="secondary" onClick={onHide}>Cancelar</Button>
                    <Button type="submit" variant="primary" disabled={(!propertyType && !isEditing) || isSubmitting}>
                        {isSubmitting ? 'Creando...' : (isEditing ? "Guardar Cambios" : "Crear Propiedad")}
                    </Button>
                </Modal.Footer>
            </Form>
        </Modal>
